$(document).ready(function(){
	var DOMAIN = "http://localhost:81/InventoryManagement/public_html"
  //alert('Hello');
...
function fetch_customer_for_reports(){
	$.ajax({
		url : DOMAIN + "/includes/process.php",
		method:"POST",
		data:{getCustomer:1,},
		success: function(data){
			...
			$("#customer_name_for_reports").html(root+data);
		}
	})
}
fetch_customer_for_reports();

function fetch_supplier_for_reports(){
	$.ajax({
		url : DOMAIN + "/includes/process.php",
		method:"POST",
		data:{getSupplierForInvoice:1},
		success: function(data){
			var root="<option value=''>Supplier Name</option>"
			$("#supplier_name_for_reports").html(root+data);
		}
	})
}

fetch_supplier_for_reports();

$("#customer_reports").off("submit").on("submit",function(){
		//alert("Hello");
		var sales=$("#customer_reports").serialize();
		window.location.href = DOMAIN+"/includes/reports_customers.php?"+sales; 
	})

$("#supplier_reports").off("submit").on("submit",function(){
		//alert("Hello");
		var sales=$("#supplier_reports").serialize();
		window.location.href = DOMAIN+"/includes/reports_suppliers.php?"+sales; 
	})
