...
public function getRecordInvoiceDetails($invoice_no,$product_name)
	...
	public function addInvoice($customer_name,$order_date,$ar_tqty,$ar_qty,$ar_qty_cat,$ar_price,$ar_pro_name,$sub_total,$gst,$discount,$net_total,$paid,$due,$payment_type)
	{
		...
		if($invoice_no!=null)
		{
			for ($i=0; $i <count($ar_pro_name) ; $i++) { 
				$insert_stmt=$this->conn->prepare("INSERT INTO `invoice_details`(`invoice_no`, `product_name`, `price`, `qty`,`unit`) VALUES (?,?,?,?,?)");
				$insert_stmt->bind_param("isdis",$invoice_no,$ar_pro_name[$i],$ar_price[$i],$ar_qty[$i],$ar_qty_cat[$i]);
	...
	public function updateInvoice($chalan_no,$customer_name,$order_date,$ar_tqty,$ar_qty,$ar_qty_cat,$ar_price,$ar_pro_name,$sub_total,$gst,$discount,$net_total,$paid,$due,$payment_type)
	{
		$order_date = date("Y-m-d", strtotime($order_date));
		...
		if($result)
		{
			for ($i=0; $i <count($ar_pro_name) ; $i++) { 
				$insert_stmt=$this->conn->prepare("INSERT INTO `invoice_details`(`invoice_no`, `product_name`, `price`, `qty`,`unit`) VALUES (?,?,?,?,?)");
				$insert_stmt->bind_param("isdis",$chalan_no,$ar_pro_name[$i],$ar_price[$i],$ar_qty[$i],$ar_qty_cat[$i]);
				$result_product_insert=$insert_stmt->execute() or die($this->conn->error);
